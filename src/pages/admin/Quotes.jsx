...
export default function Quotes() {
  ...
  const columns = [
    ...
    { header: language === 'EN' ? 'Date' : 'Datum', accessor: 'date' },
    {
      ...
      style: { minWidth: '330px', textAlign: 'right' },
      render: (row) => (
        <div className="flex flex-wrap items-center justify-start sm:justify-end gap-1.5 sm:gap-1">
          {row.status !== 'Geaccepteerd' && row.status !== 'Accepted' && (
            <button 
              onClick={() => handleConvertToProject(row)}
              className="px-2 py-1 bg-primary text-cream hover:bg-primary-dark rounded-md text-[11px] font-bold flex items-center gap-1 transition-all shadow-xs"
          ...
          <button 
            ...
            className="px-2 py-1 bg-[#EDE8DF] hover:bg-[#D6CFC2] text-primary rounded-md text-[11px] font-bold flex items-center gap-1 transition-colors"
          ...
          <button 
            ...
            className="px-2 py-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 rounded-md text-[11px] font-bold flex items-center gap-1 transition-colors"
            title="Copy Public Digital Approval Link"
          >
            <Share2 className="w-3 h-3 text-emerald-700" /> Link
          </button>
          <a
            href={`/offerte/${row.id}`}
            target="_blank"
            rel="noopener noreferrer"
            className="p-1.5 text-primary hover:bg-primary/10 rounded-md transition-colors inline-flex items-center justify-center"
          ...
          <button 
            ...
            className="px-2 py-1 bg-amber-100 hover:bg-amber-200 text-amber-900 rounded-md text-[11px] font-bold flex items-center gap-1 transition-colors"
            title={language === 'EN' ? 'Duplicate Quote (Create copy)' : 'Offerte Kopiëren'}
          >
            <Copy className="w-3 h-3 text-amber-700" /> Copy
          </button>
          <button 
            onClick={() => handleOpenEditModal(row)}
            className="p-1.5 text-dark/70 hover:text-dark hover:bg-dark/10 rounded-md transition-colors"
            title="Edit Quote"
          >
