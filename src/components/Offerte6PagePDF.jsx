...
export default function Offerte6PagePDF({ quote }) {
  ...
  const dimensions = config.dimensions || quote?.dimensions || '240 × 80 cm';
  const optionsTitle = config.optionsTitle || 'Big Green Egg Large';
  const deliveryTime = config.deliveryTime || '3 tot 5 weken';

  const subtitleText = cover.subtitleOverrideEnabled && cover.customSubtitle
    ? cover.customSubtitle
    : `${woodType} · ${dimensions} · ${optionsTitle}`;

  const coverPhotos = (cover.photos && cover.photos.length === 3)
    ? cover.photos
  ...
  const items = rawItems.length > 0 ? rawItems : [
    {
      description: `Buitenkeuken ${woodType} · ${dimensions}`,
      subtext: `Houten bovenblad met keramische stenen en uitsparing voor ${optionsTitle}`,
      quantity: 1,
      priceInclVat: 3495,
      vatRate: 21,
      isIncluded: false
    ...
    {
      description: `Bezorging ${city}`,
      subtext: `Gratis bezorgd in ${city}, op een moment dat jou uitkomt`,
      quantity: 1,
      priceInclVat: 0,
      vatRate: 21,
      isIncluded: true
  ...
  const deliveryItem = items.find(i => (i.title || i.description || '').toLowerCase().includes('bezorging'));
  ...
  const totalIncl = totals.totalInclVat || 3495;
  const totalExcl = totals.subtotalExclVat || 2888.43;
  const vatAmount = totals.vatAmount || 606.57;
  ...
  const finishTreatment = quote?.investment?.finishTreatment || 'Olieafwerking in twee lagen (naturel)';
  ...
  const specifications = config.specifications || [
    {
      ...
      title: 'BOVENBLAD',
      lines: [
        { text: 'Keramische stenen in het werkblad – hittebestendig en onderhoudsarm' },
        { text: `Uitsparing op maat voor ${optionsTitle}` }
      ]
    },
    {
      id: 'sec-2',
      ...
      lines: [
        { text: 'Twee ruime opbergvakken met deurtjes en soft-close scharnieren' },
        { text: 'Open schap voor houtopslag' }
      ]
    },
    {
      id: 'sec-3',
      ...
      lines: [
        ...
        { text: 'Verborgen heavy-duty zwenkwielen voor eenvoudige verplaatsing' }
      ]
    },
    {
      id: 'sec-4',
      title: 'BEZORGING',
      lines: [
        { text: isFreeDelivery ? `Gratis bezorgd in ${city}, op een moment dat jou uitkomt` : `Bezorging in ${city}` }
      ]
    }
  ];
  ...
  const diagram = config.diagram || {
    ...
    totalWidth: 240,
    segments: [
      { type: 'CABINET', label: 'kastje', width: 60 },
      { type: 'CABINET', label: 'kastje', width: 60 },
      { type: 'CUTOUT', label: 'Big Green Egg', width: 70 },
      { type: 'CABINET', label: 'kastje', width: 50 }
    ]
  };
      ...
      <div className="offerte-pdf-page bg-[#33422C] text-[#FDFBF7] p-6 sm:p-8 space-y-4 relative rounded-xl shadow-2xl print:rounded-none print:shadow-none h-[1050px] flex flex-col justify-between overflow-hidden">
          ...
          <div className="space-y-3 pt-12 sm:pt-16">
            <div className="space-y-1.5">
              <span className="text-xs font-mono text-[#D6CFC2] tracking-wider uppercase block font-semibold">
                VOORSTEL OP MAAT &nbsp;·&nbsp; <span className="text-[#D97706] font-bold">{quoteId}</span>
              </span>
        ...
        <div className="space-y-4 relative z-10">
          <div className="pt-4 border-t border-[#4E5E45]/80 grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs font-mono">
            <div>
              <span className="text-[10px] text-[#A19888] uppercase block tracking-wider font-bold">OPGESTELD VOOR</span>
              <span className="font-bold text-[#D97706] text-xs sm:text-sm block mt-0.5">{customerName}</span>
            </div>
            <div>
              <span className="text-[10px] text-[#A19888] uppercase block tracking-wider font-bold">OFFERTENUMMER</span>
              <span className="font-bold text-[#D97706] text-xs sm:text-sm block mt-0.5">{quoteId}</span>
            </div>
            <div>
              <span className="text-[10px] text-[#A19888] uppercase block tracking-wider font-bold">DATUM</span>
              <span className="font-bold text-[#D97706] text-xs sm:text-sm block mt-0.5">{quoteDate}</span>
            </div>
            <div>
              <span className="text-[10px] text-[#A19888] uppercase block tracking-wider font-bold">GELDIG T/M</span>
          ...
          <div className="grid grid-cols-3 gap-[2px] -mx-6 sm:-mx-10 mt-4 mb-2">
            {coverPhotos.map((pImg, idx) => (
              <img key={idx} src={pImg} alt={`Cover Photo ${idx + 1}`} className="h-40 sm:h-52 w-full object-cover" />
            ))}
          </div>
      ...
      <div className="offerte-pdf-page bg-[#FDFBF7] text-dark p-6 sm:p-8 space-y-4 rounded-xl shadow-xl print:rounded-none print:shadow-none h-[1050px] flex flex-col justify-between border border-[#C4BEB3]">
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-5 items-start">
              <div className="col-span-2 space-y-2.5 text-xs leading-relaxed text-dark/80">
                <h3 className="text-2xl sm:text-3xl font-serif text-primary" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
                  Beste <span className="text-[#D97706] font-serif">{firstName}</span>,
                </h3>
                ...
                <div className="pt-1">
                  ...
                  <p className="text-[10px] text-accent font-bold font-mono tracking-widest uppercase">OPRICHTERS VANUIT AMBACHT</p>
                </div>
              </div>
          ...
          <div className="pt-3 border-t border-[#C4BEB3] space-y-3">
            ...
            <h4 className="text-xl sm:text-2xl font-serif text-primary font-normal" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}>Waar je op kunt rekenen</h4>

            <div className="grid grid-cols-2 gap-3">
              {(quote?.letterAndProcess?.uspCards || [
                { title: 'VAKSPECIALISTEN', desc: 'De bouw ligt altijd bij gecertificeerde vakspecialisten uit ons landelijke netwerk. Vakwerk, van fundering tot afwerking.' },
                { title: 'ÉÉN AANSPREEKPUNT', desc: 'Je schakelt rechtstreeks met Tim of Bram, via WhatsApp, mail of telefoon. Korte lijnen, snelle antwoorden.' },
                { title: 'GARANTIE ÉN NAZORG', desc: 'Garantie op de constructie en nazorg na oplevering. Ook als het verblijf er staat, blijven wij je aanspreekpunt.' },
                { title: 'BEWUST ONLINE', desc: 'Geen showroom is een bewuste keuze. Zo betaal je voor vakwerk en materiaal, niet voor overhead.' }
              ]).map((card, idx) => (
      ...
      <div className="offerte-pdf-page bg-[#FDFBF7] text-dark p-6 sm:p-8 space-y-4 rounded-xl shadow-xl print:rounded-none print:shadow-none h-[1050px] flex flex-col justify-between border border-[#C4BEB3]">
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <span className="text-[10px] font-mono text-accent font-bold uppercase tracking-wider block">03 · UW CONFIGURATIE</span>
            <h3 className="text-2xl sm:text-3xl font-serif text-primary font-normal" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}>Jouw buitenkeuken in één oogopslag</h3>
          </div>

          {/* 4 Green Stat Tiles */}
          <div className="grid grid-cols-4 gap-2.5">
            <div className="bg-[#35442E] p-3.5 rounded-2xl text-center space-y-1 shadow-sm border border-[#43543A]">
              <span className="text-[9px] uppercase font-mono tracking-widest text-[#D97706] block font-bold">AFMETING</span>
              <p className="text-lg sm:text-xl font-serif text-[#D97706] leading-tight font-normal" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}>{dimensions}</p>
              <span className="text-[9px] text-[#E5DFD5] block font-body pt-0.5">centimeter</span>
            </div>

            <div className="bg-[#35442E] p-3.5 rounded-2xl text-center space-y-1 shadow-sm border border-[#43543A]">
              <span className="text-[9px] uppercase font-mono tracking-widest text-[#D97706] block font-bold">HOUTSOORT</span>
              ...
              <span className="text-[9px] text-[#E5DFD5] block font-body pt-0.5">{config.woodLifespan || '20 tot 25 jaar'}</span>
            </div>

            <div className="bg-[#35442E] p-3.5 rounded-2xl text-center space-y-1 shadow-sm border border-[#43543A]">
              <span className="text-[9px] uppercase font-mono tracking-widest text-[#D97706] block font-bold">UITSPARING / OPTIES</span>
              ...
              <span className="text-[9px] text-[#E5DFD5] block font-body pt-0.5">rechts van het midden</span>
            </div>

            <div className="bg-[#35442E] p-3.5 rounded-2xl text-center space-y-1 shadow-sm border border-[#43543A]">
              <span className="text-[9px] uppercase font-mono tracking-widest text-[#D97706] block font-bold">LEVERTIJD</span>
              ...
              <span className="text-[9px] text-[#E5DFD5] block font-body pt-0.5">na akkoord op tekening</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-5 items-start">
            ...
            {/* Photo & Diagram */}
            <div className="space-y-3">
              <div className="relative rounded-xl overflow-hidden border border-[#D6CFC2] shadow-xs">
                <img src={config.configPhoto || projectImg} alt="Configuration" className="h-36 w-full object-cover" />
              </div>

              {/* Front-View Diagram output */}
              {diagram.show && (
                <div className="p-3 bg-[#F4EFE6] rounded-2xl border border-[#E2DDD3] text-center space-y-2 shadow-xs">
                  <span className="text-[9px] font-mono uppercase font-bold text-accent tracking-widest block">VOORAANZEICHT TEKENING ({diagram.totalWidth} cm)</span>
